...
class Task(PowerAware):
    ...
    def allocate(self, node: Node):
        ...
        if self.node is not None:  # If self.node is not None, it means the task is already placed
            raise ValueError(f"Cannot place {self} on {node}: It was already placed on {self.node}.")
        self.node = node
        self.node._add_task(self)  # add task to node
    ...
    def measure_power(self) -> PowerMeasurement:
        try:
            #if added by reza
            #Sensor nodes (Source and Sink Tasks) do not use much power
            if isinstance(self,SourceTask) or isinstance(self,SinkTask):
                #return PowerMeasurement(0, self.cu)
...
class ProcessingTask(Task):
    def __init__(self, cu: float = 0, scheduling_id: int = None):
...
class Application(PowerAware):
    ...
    def read_data_from_csv(self, file_path2):
        device_capacities = []
        tasks_instructions = []
